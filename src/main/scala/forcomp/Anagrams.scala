...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.filter(_.isLetter).groupBy(identity).mapValues(_.length).toList.sorted
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = (x.toSet -- y).toList
